export default {
  nav: {
    home: "Home",
    projects: "Projects",
    about: "About",
    contact: "Contact",
  },
  hero: {
    title: "Full Stack Developer",
    subtitle: "Building fast, accessible and delightful web experiences.",
    cta1: "View Work",
    cta2: "Contact Me",
    badge: "Available for new opportunities",
  },
  footer: {
    rights: "All rights reserved.",
    titleLine1: "Let's create",
    titleLine2: "something epic.",
    emailLabel: "Email me",
    available: "Available for Freelance",
    location: "Caracas, Venezuela",
    privacy: "Privacy",
    terms: "Terms",
    backToTop: "Back to Top",
    whatsapp: {
      title: "Chat on WhatsApp",
      number: "+58 412 7893937",
      action: "Open Chat",
    },
  },
  projects: {
    title: "Projects",
    viewDetails: "View Details",
    garcias: {
      description:
        "Full-stack web application for a professional installation services company.",
      longDescription:
        "A comprehensive digital solution for a local installation business...",
      feature1: "Real-time Quote System",
      feature2: "Admin Dashboard",
      feature3: "Image Optimization",
      feature4: "Responsive Design",
    },
    hph: {
      description:
        "Dynamic modular web forum system with multimedia content...",
      longDescription:
        "A customized Simple Machines Forum (SMF) implementation...",
      feature1: "Custom SMF Themes",
      feature2: "Gamification System",
      feature3: "High Performance",
      feature4: "Community Modules",
    },
    portfolio: {
      title: "Full Stack Portfolio",
      description:
        "Professional portfolio website showcasing projects and skills.",
      longDescription:
        "My personal showcase designed to demonstrate modern web development capabilities...",
      feature1: "Server-Side Rendering",
      feature2: "Internationalization",
      feature3: "Perfect Lighthouse Score",
      feature4: "Glassmorphism UI",
    },
  },
  experience: {
    title: "Professional Journey",
    intro:
      "I design and build real-world products with a strong focus on performance, clarity, and long-term maintainability.",
    meta: "Selected experience",
    exp1: {
      role: "Full Stack Developer",
      date: "Nov 2024 — Present",
      narrative:
        "Working on a live production platform with thousands of users...",
      highlight1: "End-to-end feature development across frontend and backend",
      highlight2: "Refactoring legacy codebases to improve scalability",
      highlight3: "Performance, accessibility, and technical SEO optimization",
      highlight4: "Architecture decisions and production deployments",
      focus1: "Production systems",
      focus2: "Performance",
      focus3: "SEO",
      focus4: "Code quality",
    },
    exp2: {
      role: "Full Stack Programming Student",
      company: "University Full Stack Program",
      date: "2023 — 2024",
      narrative: "Completed a university-backed full stack program...",
      highlight1: "Built full-stack applications from scratch",
      highlight2: "Learned clean code and deployment best practices",
      highlight3: "Worked with modern frameworks and tooling",
      focus1: "Foundations",
      focus2: "Modern stack",
      focus3: "Best practices",
    },
  },
  contact: {
    title: "Let’s create something meaningful.",
    subtitle:
      "If you have an idea, a project, or just a thought — write to me.",
    note: "Direct communication. No agencies. Reply within 24h.",
    name: "Your name",
    email: "Email",
    messageLabel: "Message",
    message: "Tell me what’s on your mind…",
    send: "Send message",
    sending: "Sending...",
    success: "✓ Message sent successfully! I'll get back to you soon.",
    error: "✗ Failed to send message. Please try again.",
  },
  certificates: {
    verified: "Verified Credential",
    certified: "Certified Professional",
    title: "Full Stack Developer",
    description:
      "8 months of intensive training, mastering modern software architectures. From responsive interface design to scalable server deployment.",
    months: "Intensive Months",
    hours: "Coding Hours",
    stack: "Mastered Arsenal",
    view: "View Credential",
  },
  about: {
    title: "About Me",
    p1: "I am passionate about building scalable, maintainable, and emotionally resonant web systems.",
    p2: "My focus is on performance, accessibility, and premium design experiences.",
    location: "Caracas, Venezuela",
    status: "Full Stack Developer",
    ctaTalk: "Let's talk",
    ctaDownload: "Download CV",
    openToWork: "Open to work",
    stackLabel: "Stack",
    stackTitle: "Technologies & Tools",
  },
  github: {
    title: "GitHub Journey",
    description:
      "Explore my code contributions, daily activity, and open source projects. I believe in consistent improvement and transparent development.",
    cta: "Visit GitHub Profile",
    commits: "Total Commits",
    hours: "Hours Coded",
    streak: "Longest Streak",
  },
  codegallery: {
    meta: "Technical Excellence",
    title: "Software Engineering",
    subtitle: "Beyond code: building digital solutions that define the future.",
    card1: {
      title: "Clean Architecture",
      subtitle:
        "Robust and scalable design patterns for enterprise applications.",
    },
    card2: {
      title: "Optimization",
      subtitle: "Maximum performance and instant load times.",
    },
    card3: {
      title: "Security",
      subtitle: "Implementation of best practices and data protection.",
    },
    card4: {
      title: "Innovation",
      subtitle: "Use of the latest technologies in the web ecosystem.",
    },
  },
};
